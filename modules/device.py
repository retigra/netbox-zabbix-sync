"""
Device specific handling for NetBox to Zabbix
"""
...
from modules.interface import InterfaceTypes, ZabbixInterface
...
class PhysicalDevice:
    ...
    def cleanup(self):
        ...
        if self.zabbix_id:
            try:
                ...
                message = f"Zabbix returned the following error: {e!s}."
    ...
    def set_interface_details(self):
        ...
        try:
            ...
            if interface.get_context():
                ...
                if interface.interface["type"] == InterfaceTypes.SNMP:
                    interface.set_snmp()
            else:
    ...
    def create_in_zabbix(
        ...
        if not self._zabbix_hostname_exists():
            ...
            try:
                ...
                msg = f"Host {self.name}: Couldn't create. Zabbix returned {e!s}."
        ...
        for hostgroup in self.hostgroups:
            for pos in range(len(hostgroup.split("/"))):
                ...
                try:
                    ...
                    msg = f"Hostgroup '{zabbix_hg}': unable to create. Zabbix returned {e!s}."
                    self.logger.error(msg)
                    raise SyncExternalError(msg) from e
        return final_data
    ...
    def update_zabbix_host(self, **kwargs):
        ...
        try:
            ...
            e = (
                ...
                f"Zabbix returned the following error: {e!s}."
            )
            self.logger.error(e)
    ...
    def consistency_check(
        ...
        if not self.set_zabbix_group_id(
            groups
            ...
            if (
                not self.group_ids
                and not self.set_zabbix_group_id(groups)
                and not create_hostgroups
            ):
                e = (
            ...
            if updates:
                ...
                if "type" in updates:
                    ...
                    e = (
                        ...
                        f"{updates['type']!s} is not supported."
                    )
                    self.logger.error(e)
                ...
                try:
                    ...
                    msg = f"Zabbix returned the following error: {e!s}."
                    self.logger.error(msg)
                    raise SyncExternalError(msg) from e
            else:
